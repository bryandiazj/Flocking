...
bool addToListOfAttractionPts = false;
bool addInfectedAgent = false;
...
void menu(int num) {
  ...
  if(num==-1) {
    ...
    addIndividual=true;
    addGroup=false;
    addToListOfAttractionPts = false;
    addAttractionPt = false;
    addInfectedAgent = false;
  }
  else if(num==2) {
    addIndividual=false;
    addGroup=true;
...
void createMenu() {
    ...
    glutAddMenuEntry("Add List of Attraction Point", 4);
    glutAddMenuEntry("Add Infected Agent", 5);
...
void mousebutton(int button, int state, int x, int y)
{
  if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
    ...
    if( addIndividual )
      gSim.AddMember(-1,tx,ty, HEALTHY);
    else if( addGroup )
      for(int i=0; i<5; i++)
	gSim.AddMember(-1,tx,ty, HEALTHY);
    ...
    else if (addToListOfAttractionPts) {
    }

    else if (addInfectedAgent) {
        gSim.AddMember(-1, tx, ty, INFECTED);
    }
