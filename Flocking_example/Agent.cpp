...
void Agent::Init(int _id, Vector3d _pos, Vector3d _vel, double _mass, 
                 double _maxVel, double _maxAccel, double _viewRadius, HealthStatus _hs){
...
void Agent::Update(vector<Agent>& agents, double dt) {
  ...
  if( isControlled ) {
    ...
    if (dist < (agents[i].GetRadius() + GetRadius() + epsilon)&& healthStatus == INFECTED) {
        agents[i].SetHealth(INFECTED);
        num--;

        //transmission
    }

    if (dist < (agents[i].GetRadius() + GetRadius() + epsilon) && healthStatus == HEALTHY) {
        num++;
  ...
  if( cdWorld ) {
    ...
    if( updated ) {
        ...
        if (pNew.GetY() < 0.0) {//forward
            Vector3d dir = vel.normalize();
            force = dir * 10.0 * maxAccel;
        }
        else if (pNew.GetY() > 0.0) {//backward
            Vector3d dir = vel.normalize();
            force = dir * (-10.0 * maxAccel);
        }
        else if (pNew.GetX() > 0.0) {//left
            Vector3d dir = vel.normalize();
            force[0] = dir[1];
            force[1] = -1 * dir[0];
            force *= -10.0 * maxAccel;
        }
        else if (pNew.GetX() < 0.0) {//right
            Vector3d dir = vel.normalize();
            force[0] = dir[1];
            force[1] = -1 * dir[0];
            force *= 10 * maxAccel;
...
void drawTriangle_2(double,bool);
void drawTriangleStatus_2(double, double);

void drawAgentAsTriangle(double radius, double divisions, bool filled, double percStatus) {

    drawCircle(radius, divisions, false);

    drawCircle(percStatus * radius, divisions, filled);

    drawTriangle_2(divisions, false);

    drawTriangle_2(divisions, filled);


    //drawTriangleStatus_2(divisions, percStatus);
}
...
void drawTriangle_2(double length, bool filled) {
    if (!filled)
        glBegin(GL_LINE_LOOP);
    else
        glBegin(GL_TRIANGLES);
    
    //glVertex2f((length / 2.0)-17, 0);
    //glVertex2f((-length / 2.0)-17, (length / 3.0));
    //glVertex2f((-length / 2.0)-17, (-length / 3.0));
...
void drawTriangleStatus_2(double length, double percStatus) {
    glBegin(GL_LINE_LOOP);
    glVertex2f((-length / 2.0)-17, length / 3.0);
    glVertex2f((-length / 2.0)-17, -length / 3.0);
    glVertex2f((-length / 2.0 - 5)-17, -length / 3.0);
    glVertex2f((-length / 2.0 - 5)-17, length / 3.0);
    glEnd();
    ...
    glVertex2f((-length / 2.0 - 5)-17, length / 3.0);
    glVertex2f((-length / 2.0)-17, length / 3.0);
    glVertex2f((-length / 2.0)-17, length / 3.0 - del);
    glVertex2f((-length / 2.0 - 5)-17, length / 3.0 - del);
    glEnd();
}
...
void Agent::Draw() {
  ...
  */
  if( drawMode == 1 ) {
    ...
    //drawAgentAsTriangle(radius, 20.0, isControlled, 1.0*status/maxStatus);


   drawCircle(radius, 20.0, false);

    drawCircle((1.0 * status / maxStatus) * radius, 20.0, isControlled);
    ...
    drawTriangle_2(20.0, false);

    drawTriangle_2(20.0, isControlled);

    drawTriangleStatus_2(20.0, 1.0 * status / maxStatus);
    ...
    glScalef(10.0f, 1.0f, 0);


    if (num < bar*mult) {
        if (num > 0.0)
        drawHealth3(num/mult);
        else 
            drawHealth3(0.0);
    }

    else if (num > bar * mult) {
        if (num / mult < 50.0)
            drawHealth3(num / mult);
        else
            drawHealth3(50.0);
